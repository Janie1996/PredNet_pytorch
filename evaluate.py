...
# zcr lib
from PredNet_pytorch.prednet import PredNet
from PredNet_pytorch.data_utils import ZcrDataLoader
from Video_Prediction.MovingMNIST import MovingMNIST
...
def arg_parse():
    ...
    parser.add_argument('--checkpoint_file', default = 'E:\\prenet.pkl', type = str,
...
def evaluate(model, args):
    ...
    test_set = MovingMNIST(root='data/mnist', train=False, nt=10, download=True)
    ...
    for step, (frameGroup, target) in enumerate(test_loader):
        X_test=frameGroup.unsqueeze(2)
        break
    ...
    for t in range(X_groundTruth.shape[0]):
        X_groundTruth_list.append(X_groundTruth[t, ...])    # (batch_size, 3, 128, 160)
    ...
    for i in range(2):
        for t in range(3):
            ...
            plt.subplot(gs[t])
            truth=unloader(torch.from_numpy(X_groundTruth_list[t][i, ...]))
            ...
            truth = unloader(torch.from_numpy(X_predict_list[t][i, ...]))
            plt.imshow(truth, interpolation='none')
            #plt.show()
            #plt.imshow(X_predict_list[t][i,0,...], interpolation = 'none')
            plt.tick_params(axis = 'both', which = 'both', bottom = 'off', top = 'off', left = 'off', right = 'off', labelbottom = 'off', labelleft = 'off')
            if t == 0:
