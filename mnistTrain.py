...
def arg_parse():
    ...
    parser.add_argument('--dataPath', default='E:\\PycharmProjects\\untitled\\PredNet_pytorch\\kitti_data', type=str,
                        metavar='PATH',
    ...
    parser.add_argument('--printCircle', default=1000, type=int, metavar='N',
                        help='how many steps to print the loss information')
    parser.add_argument('--data_format', default='channels_first', type=str,
                        help='(c, h, w) or (h, w, c)?')
    parser.add_argument('--n_channels', default=1, type=int, metavar='N',
                        help='The number of input channels (default: 3)')
    parser.add_argument('--img_height', default=64, type=int, metavar='N',
                        help='The height of input frame (default: 128)')
    parser.add_argument('--img_width', default=64, type=int, metavar='N',
...
def train(model, args):
    ...
    train_set = MovingMNIST(root='data/mnist', train=True, nt=10, download=True)
    ...
    lr_maker = lr_scheduler.StepLR(optimizer=optimizer, step_size=75,
                                   gamma=0.1)  # decay the lr every 50 epochs by a factor of 0.1
    ...
    for e in range(args.epochs):
        ...
        initial_states = prednet.get_initial_states(
            input_shape)  # 原网络貌似不是stateful的, 故这里再每个epoch开始时重新初始化(如果是stateful的, 则只在全部的epoch开始时初始化一次)
        states = initial_states
        for step, (frameGroup, target) in enumerate(train_loader):
            # print(frameGroup)   # [torch.FloatTensor of size 16x12x80x80]
            batch_frames = Variable(frameGroup.unsqueeze(2).cuda())  # .half()) # .half()
            ...
            layer_weights = Variable(
                layer_weights.float().cuda())  # NOTE: layer_weights默认是DoubleTensor, 而下面的error是FloatTensor的Variable, 如果直接相乘会报错!
            ...
            if step % printCircle == (printCircle - 1):
                print('epoch: [%3d/%3d] | [%4d/%4d]  loss: %.4f  lr: %.5lf' % (
                (e + 1), args.epochs, (step + 1), len(train_loader), tr_loss / printCircle,
...
def saveCheckpoint(zcr_state_dict, fileName='./checkpoint/checkpoint_newest.pkl'):
    ...
    fileName = 'E:\\' + str(epoch) + '_trLoss' + tr_loss + '.pkl'
...
if __name__ == '__main__':
    ...
    # DATA_DIR = args.dataPath
    # data_file = os.path.join(DATA_DIR, 'X_val.h5')
    # source_file = os.path.join(DATA_DIR, 'sources_val.h5')
    # output_mode = 'error'
    # sequence_start_mode = 'all'
    # N_seq = None
